...
# --- OS & Distro Detection (ZSH) ---
if [[ -f /etc/os-release ]]; then
    source /etc/os-release
    ZSH_DISTRO_FAMILY="unknown"
    if [[ "$ID" == "ubuntu" || "$ID" == "debian" || "$ID_LIKE" == *"ubuntu"* || "$ID_LIKE" == *"debian"* ]]; then
        ZSH_DISTRO_FAMILY="debian"
    elif [[ "$ID" == "fedora" || "$ID_LIKE" == *"fedora"* ]]; then
        ZSH_DISTRO_FAMILY="fedora"
    elif [[ "$ID" == "arch" || "$ID_LIKE" == *"arch"* ]]; then
        ZSH_DISTRO_FAMILY="arch"
    fi
fi

# --- Package Management (Distro Aware) ---
case "$ZSH_DISTRO_FAMILY" in
    debian)
        ...
        alias pki-show="apt show"
        alias pki-list="dpkg -l | grep"
        ;;
    fedora)
        alias update="sudo dnf update && (command -v brew >/dev/null 2>&1 && brew update && brew upgrade || true)"
        alias pki="sudo dnf install"
        alias pkr="sudo dnf remove"
        alias pks="dnf search"
        alias pki-show="dnf info"
        alias pki-list="rpm -qa | grep"
        ;;
    arch)
        alias update="sudo pacman -Syu && (command -v brew >/dev/null 2>&1 && brew update && brew upgrade || true)"
        alias pki="sudo pacman -S"
        alias pkr="sudo pacman -Rs"
        alias pks="pacman -Ss"
        alias pki-show="pacman -Si"
        alias pki-list="pacman -Qs"
        ...
        ;;
    *)
        alias update="(command -v brew >/dev/null 2>&1 && brew update && brew upgrade || true)"
        ;;
esac
