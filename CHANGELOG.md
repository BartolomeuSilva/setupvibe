...
- **Full Fedora Support**: Added support for Fedora 41+ (DNF) in both Desktop and Server editions.
- **Full Arch/Manjaro Support**: Added support for Arch-based distributions (Pacman) in both Desktop and Server editions.
- **Package Management Abstraction**: Introduced `pkg_install` and `pkg_update` helper functions for distribution-agnostic package management.
- **Dynamic ZSH Aliases**: Rewrote `zshrc-linux.zsh` and `zshrc-server.zsh` to detect distribution at login and provide appropriate aliases (`pki`, `pkr`, `pks`, etc.).
- **Automatic AUR Helper**: Added automatic `yay` installation on Arch-based systems.

### Changed

- Refactored OS detection logic to use `/etc/os-release` and introduced `DISTRO_FAMILY` categorization.
- Updated all installation steps to use native repository strategies for Fedora and Arch.
- Modernized documentation to reflect cross-distro capabilities.
