...
**SetupVibe** is a cross-platform automated development environment setup script (v0.42.0). It installs and configures a complete developer toolkit in one command, supporting macOS 12+ and Linux (Ubuntu/Debian, Fedora 41+, Arch/Manjaro).
...
1. Detect OS, distro, and CPU architecture (`x86_64` vs `arm64/aarch64`) and set `DISTRO_FAMILY` (`debian`, `fedora`, `arch`, `macos`)
...
4. Use package management abstraction helpers (`pkg_install`, `pkg_update`) for cross-distro compatibility
5. Clean up legacy APT repository entries (on Debian-based systems) before adding new ones
...
Subset of desktop steps — no Homebrew, no language ecosystems (PHP, Ruby, Python, Go, Rust) or desktop-specific tools. Node.js is installed via NodeSource (Debian/Fedora) or official repository (Arch). Focus on base tools, Docker, Ansible, shell, tmux, monitoring, and AI CLIs.
...
**Package Management Abstraction** — scripts use helpers to handle multiple package managers:

```bash
pkg_install build-essential git
pkg_update
```

**APT keyring management** (Debian-based) — keyrings go in `/etc/apt/keyrings/`; legacy entries in `/etc/apt/sources.list.d/` are removed before re-adding to avoid signature errors.

**Package lock waiting** (Servers) — server script polls for package manager lock release before running install commands.

**Architecture detection**:

```bash
ARCH_RAW=$(uname -m)  # x86_64 or aarch64
```

## Development Conventions

- **Helper Functions**: Use `user_do` to run commands as the real user and `sys_do` for commands requiring elevated privileges. Use `pkg_install` and `pkg_update` for package management.
...
- **Lock Management**: Especially in server environments, check for package manager locks before operations.
