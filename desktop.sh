...
# ------------------------------------------------------------------------------
# Compatibility: macOS 12+, Ubuntu 24.04+, Debian 12+, Fedora 41+, Arch/Manjaro
...
cron_ensure() {
    ...
    if [[ "$(uname -s)" == "Linux" ]]; then
        # Debian/Ubuntu use 'cron', Arch uses 'cronie', Fedora uses 'crond'
        sys_do systemctl enable --now cron 2>/dev/null || \
        sys_do systemctl enable --now cronie 2>/dev/null || \
        sys_do systemctl enable --now crond 2>/dev/null || true
    fi
...
# Helper to install GPG keys safely (APT/Debian specific mainly, but kept for compatibility)
install_key() {
    local url=$1
    local dest=$2
    [[ "$DISTRO_FAMILY" != "debian" ]] && return 0 # Only needed for Debian family APT
...
if [[ "$DISTRO_FAMILY" == "debian" ]]; then
    ...
    # Remove potentially broken lists
...
step_1() {
    if $IS_MACOS; then
        ...
        case "$DISTRO_FAMILY" in
            debian)
                echo "Installing Build Essentials & Core Tools (APT)..."
                ...
                ;;
            fedora)
                echo "Installing Development Tools & Core Tools (DNF)..."
                sys_do dnf groupinstall -y "Development Tools" "C Development Tools and Libraries"
                pkg_install git wget unzip fontconfig curl sshpass dnf-plugins-core \
                    ...
                    libyaml-devel gdbm-devel gmp-devel rust autoconf bison procps-ng file tmux ffmpeg ImageMagick-devel cronie
                ;;
            arch)
                echo "Installing Base-Devel & Core Tools (Pacman)..."
                pkg_install base-devel git wget unzip fontconfig curl sshpass \
                    openssl zlib bzip2 readline sqlite ncurses xz tk libxml2 \
                    xmlsec libffi libyaml gdbm gmp rust autoconf bison procps-ng file tmux ffmpeg imagemagick cronie
                
                # Install AUR helper (yay) if requested and not present
                if ! command -v yay &>/dev/null; then
                    echo "Installing yay (AUR helper)..."
                    user_do mkdir -p $REAL_HOME/.cache/yay-build
                    user_do git clone https://aur.archlinux.org/yay-bin.git $REAL_HOME/.cache/yay-build/yay-bin
                    ( cd $REAL_HOME/.cache/yay-build/yay-bin && user_do makepkg -si --noconfirm )
                fi
                ;;
        esac
    fi
...
step_2() {
    if $IS_MACOS; then
        ...
        if [ ! -d "/home/linuxbrew/.linuxbrew" ] && [ ! -d "$REAL_HOME/.linuxbrew" ]; then
            echo "Installing Homebrew..."
            case "$DISTRO_FAMILY" in
                debian) pkg_install build-essential procps curl file git ;;
                fedora) pkg_install git curl procps-ng file ;;
                arch)   pkg_install git curl procps-ng file ;;
            esac
...
step_3() {
    if $IS_MACOS; then
        ...
        if [[ "$DISTRO_FAMILY" == "debian" ]]; then
            ...
            pkg_update
            echo "Installing PHP 8.4 & Extensions (APT)..."
            pkg_install php8.4 php8.4-cli php8.4-common php8.4-dev \
            ...
            echo "Using Fedora Remi Strategy..."
            pkg_install https://rpms.remirepo.net/fedora/remi-release-$(rpm -E %fedora).rpm
            sys_do dnf config-manager --set-enabled remi-php84
            pkg_install php php-cli php-common php-devel php-curl php-mbstring php-xml php-zip \
                php-bcmath php-intl php-mysqlnd php-pgsql php-sqlite3 php-gd php-pecl-imagick \
                php-pecl-redis php-pecl-mongodb php-pecl-yaml php-pecl-xdebug
        elif [[ "$DISTRO_FAMILY" == "arch" ]]; then
            echo "Using Arch Stable Strategy..."
            pkg_install php php-gd php-intl php-embed php-redis php-mongodb php-imagick xdebug \
                composer
        else
            echo -e "${YELLOW}⚠ Unknown Linux distribution family. Attempting generic PHP install...${NC}"
            pkg_install php
        fi
...
step_4() {
    ...
    if $IS_MACOS; then
        ...
        if ! user_do bash -c 'export PATH="$HOME/.rbenv/bin:$PATH"; eval "$(rbenv init -)"; rbenv versions --bare | grep -q "^3.3.0$"'; then
            ...
            # CFLAGS="-Wno-error=incompatible-pointer-types" to handle stricter checks in GCC 14+
            user_do bash -c 'mkdir -p "$HOME/.tmp"; export TMPDIR="$HOME/.tmp"; export PATH="$HOME/.rbenv/bin:$PATH"; eval "$(rbenv init -)"; export RUBY_CONFIGURE_OPTS="--disable-install-doc --enable-shared"; export CFLAGS="-Wno-error=incompatible-pointer-types"; export MAKE_OPTS="-j$(nproc 2>/dev/null || echo 2)"; rbenv install 3.3.0 && rbenv global 3.3.0'
        fi
...
step_5() {
    if $IS_MACOS; then
        ...
        echo "Setup Python..."
        case "$DISTRO_FAMILY" in
            debian) pkg_install python3 python3-pip python3-venv python-is-python3 ;;
            fedora) pkg_install python3 python3-pip python3-devel ;;
    ...
    if $IS_MACOS; then
        ...
        if ! command -v tailscale &>/dev/null; then
            if [[ "$DISTRO_FAMILY" == "arch" ]]; then
                # Use pacman for Arch-based distros (handles unrecognized IDs like BigLinux)
