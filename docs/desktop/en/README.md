...
> Cross-platform development environment setup — v0.42.0
...
| **Ubuntu/Debian** | Current stable versions         |
| **Fedora**        | 41+                             |
| **Arch/Manjaro**  | Current stable rolling release  |
...
**Linux:** installs via native package manager (APT, DNF, or Pacman) — `build-essential` (or equivalent), `git`, `wget`, `unzip`, `curl`, `tmux`, `ffmpeg`, `imagemagick`, SSL/compression libs.
...
| ----------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| PHP 8.4           | via Homebrew                                | via ondrej/php PPA (Ubuntu), sury.org (Debian), Remi (Fedora), or official (Arch)                   |
| Extensions        | redis, xdebug, imagick via PECL             | php8.4-* modules (Debian/Ubuntu/Fedora) or official packages (Arch)                                 |
...
| -------- | -------------------------- | -------------------------------------------------- |
| Python 3 | `python@3.12` via Homebrew | via native package manager (`python3`, `python3-pip`) |
...
| Tool       | macOS                     | Linux                   |
| ---------- | ------------------------- | ----------------------- |
| Node.js 24 | `node@24` via Homebrew    | via NodeSource (Debian/Fedora) or official (Arch) |
...
| Tool              | macOS                            | Linux                                                                        |
| ----------------- | -------------------------------- | ---------------------------------------------------------------------------- |
| Docker            | Docker Desktop via Homebrew Cask | docker-ce via official Docker repo (Debian/Fedora) or official package (Arch)        |
| Portainer         | via Docker Compose in `~/.setupvibe` | same |
| Ansible           | via Homebrew                     | via PPA (Ubuntu), ansible-core (Debian), or official (Fedora/Arch)           |
| GitHub CLI (`gh`) | via Homebrew                     | via official repo (Debian/Fedora) or official package (Arch)                 |
...
**Linux:** native cleanup (autoremove, clean), removes temp archives and vacuums journal logs; clears `~/.cache/pip`, `~/.cache/composer`, `~/.npm/_npx`, `~/.bundle/cache`.
...
| `reload`   | `source ~/.zshrc`                                                                     |
| `zconfig`  | `nano ~/.zshrc`                                                                       |
| `update`   | `brew update && brew upgrade` (macOS) / `update` (Linux - distro aware)               |
