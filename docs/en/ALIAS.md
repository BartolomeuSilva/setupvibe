...
> Shell environment aliases — v0.42.0
...
- **`update`**
  - Availability: 🌐 Both
  - Command: `brew update && brew upgrade` (macOS) / `sudo apt update...` (Debian) / `sudo dnf update` (Fedora) / `sudo pacman -Syu` (Arch)
  - Description: Updates the system and package managers. Distro-aware on Linux.
  - Example: `update`

- **`pki`**
  - Availability: 🌐 Both (Linux)
  - Command: `sudo apt install` (Debian) / `sudo dnf install` (Fedora) / `sudo pacman -S` (Arch)
  - Description: Installs a package. Distro-aware.
  - Example: `pki htop`

- **`pkr`**
  - Availability: 🌐 Both (Linux)
  - Command: `sudo apt remove` (Debian) / `sudo dnf remove` (Fedora) / `sudo pacman -Rs` (Arch)
  - Description: Removes a package. Distro-aware.
  - Example: `pkr htop`

- **`pks`**
  - Availability: 🌐 Both (Linux)
  - Command: `apt search` (Debian) / `dnf search` (Fedora) / `pacman -Ss` (Arch)
  - Description: Searches repositories for packages. Distro-aware.
  - Example: `pks nginx`

- **`pki-show`**
  - Availability: 🌐 Both (Linux)
  - Command: `apt show` (Debian) / `dnf info` (Fedora) / `pacman -Si` (Arch)
  - Description: Displays package details. Distro-aware.
  - Example: `pki-show git`

- **`pki-list`**
  - Availability: 🌐 Both (Linux)
  - Command: `dpkg -l` (Debian) / `rpm -qa` (Fedora) / `pacman -Qs` (Arch)
  - Description: Lists installed packages. Distro-aware.
  - Example: `pki-list nginx`
